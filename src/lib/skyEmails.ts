...
import { atlasChart, citiesFor, roundKm, nearLabel, DEFAULT_SHOWN, LINE_NAME, QUESTIONS } from "./atlas";
...
export async function emailAtlas(self: CardMember, email: string): Promise<SendResult> {
  ...
  return sendEmail("atlas", [email], {
    ...
    groups: QUESTIONS.map((q) => {
      ...
      return { title: q.title, why: q.why, rows: citiesFor(chart, q, 4).map((r) => ({ city: nearLabel(r.city) ? `${r.city[0]} <span style="color:#8a7f66;font-size:13px;font-style:italic;">near ${nearLabel(r.city)}</span>` : r.city[0], detail: `${planet.name} ${LINE_NAME[r.kind]} line passes ${roundKm(r.km)} km away`, strength: r.strength })) };
    }),
  });
}
