...
- **Member cards** — every member avatar across the app (Convene, Oracle, Tribunal, Profile roster) is clickable to a tarot-style stats card: portrait (click it again to enlarge in a small lightbox), name, their **title** if they have chosen one (otherwise **moon phase at birth**), role, the birth-derived fields (tooltipped, never the raw date/time of birth) — element, sun, moon, ascendant, Shengxiao, Wu Xing, **Bazi day-master, life path number, Venus sign, and birth arcana** (each with a label tooltip *and* a per-result meaning tooltip) — **Value for coin** and **The purse** (their bottle economics against the table's, once 2+ of their bottles carry prices), and a **chalice count**, one gold chalice per codex victory. Animated open/close (`src/components/MemberCard.tsx`).
