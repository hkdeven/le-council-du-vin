...
- **Crowns survive disqualifications** — `championsOf()` in `src/lib/annals.ts` is the single source of truth for who took a night: rank 1 non-DQ as normal; a DQ'd first place is never credited and the best surviving score is promoted; an unscored night (imported history) still crowns the sole wine left standing after disqualifications. Wired through the codex crownings + victory ledger + chalice champion, member-card chalices, the dossier, and the prophecy's grade.
- **The Heavens (designed + approved, next build)**: one door replacing the three sky buttons; the member card flips over (3D turn, arrow + footer control to turn back, one close for everything) to reveal three stops — The Wheel · The Foretelling · The Kundli — with the Foretelling carrying This day/This moon/The year under a Western/Vedic lens pill (tara + panchang almanac, gochara, the year's turnings, the pratyantar clock, Sade Sati's iron passage, all verified computable with real dates). Demos: `public/heavens-flip-demo.html`, `heavens-demo.html`, `heavens-mockup.html`, `kundli-horoscope-mockup.html`. **Four heavens emails** ready in `emailTemplates.ts` (natal, foretelling now with This Day, kundli, and the new Vedic foretelling with The Day's Star); previews 7-10 under `public/email-previews/`.
